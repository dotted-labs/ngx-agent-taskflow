...
import { LocalStorageTaskService } from '../services/ngx-agent-taskflow.service';
...
export interface TaskCallbacks<T = TaskMessageTypes> {
  ...
  onTasksLoad?: () => Observable<Task<T, any>[]>;
...
export class TaskListStore extends signalStore(
  ...
  withMethods((store) => {
    const localStorageTaskService = inject(LocalStorageTaskService);
    ...
    return {
      ...
      init<T>(
        ...
        patchState(store, {
          ...
          saveInLocalStorage: options.saveInLocalStorage,
          saveInLocalStorageKey: options.saveInLocalStorageKey,
        });

        // Load existing tasks if onTasksLoad callback is provided
        if (options.callbacks.onTasksLoad) {
      ...
      createTask<T = TaskMessageTypes>(
        ...
        if (store.saveInLocalStorage()) {
          localStorageTaskService.createTask(store.saveInLocalStorageKey(), task as any);
        }
      ...
      removeTask(taskId: string) {
        ...
        if (store.saveInLocalStorage()) {
          localStorageTaskService.deleteTask(store.saveInLocalStorageKey(), taskId);
        }
      },
      ...
      updateTaskStatus(taskId: string, status: TaskStatus) {
        ...
        if (store.saveInLocalStorage()) {
          localStorageTaskService.updateTask(store.saveInLocalStorageKey(), taskId, { status });
        }
      },
       ...
       */
      sendMessage(taskId: string, messageContent: string) {
        ...
        if (store.saveInLocalStorage()) {
          localStorageTaskService.updateTask(store.saveInLocalStorageKey(), taskId, messageContent as any);
        }
      },
       ...
       */
      addTaskMessage<T, U>(taskId: string, sender: TaskMessageSender, data: TaskData<T, U>) {
        ...
        if (task) {
          ...
          if (store.saveInLocalStorage()) {
            localStorageTaskService.updateTask(store.saveInLocalStorageKey(), taskId, { messages: updatedMessages } as any);
          }
        }
      },
